...
<main class="pt-5 max-lg-5">
  ...
  { ?>
    <form action = "" method = "POST" enctype = "multipart/form-data" class="border border-light p-5">
      <p class="h4 mb-4 text-left">Ajouter une catégorie</p>
      <input class="form-control mb-4" name="Nom" id="defaultContactFormName" type="text" placeholder="Nom de la catégorie">
...
<?php
  ...
  {
    $query=$bdd->prepare('INSERT INTO categorielivre (NomCategorie)
    VALUES (:Nom)');
    $query->bindValue(':Nom', $NomCategorie, PDO::PARAM_STR);


    $query->execute();

    $NumCategorie = $bdd->query("SELECT NumCategorie FROM categorielivre WHERE NumCategorie = LAST_INSERT_ID()") -> fetch();

    echo "<script type='text/javascript'>window.location.href = 'categories_admin.php';</script>";

  }

  else {
    echo "\t", 'Il faut remplir tous les champs pour rajouter une catégorie. ';
    echo "\t", '<p>Cliquez <a href="./add_categorie.php">ici</a> pour recommencer</p>';
  }
}
