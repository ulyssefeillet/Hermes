...
<button id="btn_add">
  <a href="add_auteur.php"> <i class="fa fa-plus">&nbsp Ajouter un nouveau auteur </i></a>
</button>
...
<ul class="list-group mb-2 mb-sm-0 pt-1">
  ...
  while ($donnees = $reponse->fetch()) {  ?>
    <li class="list-group-item" id="<?php echo $donnees['NumAuteur'] ?>">

      <?php echo $donnees['NomAuteur'], " ", $donnees['PrenomAuteur'] ?>
      <div class="buttons-panel">
        <form method="POST" action="modify_auteur.php">
          <button type="submit" name="Modifier" class="btn btn-lg btn-primary btn-sm" value="<?php echo $donnees['NumAuteur'] ?>">Modifier</button>
        </form>
        <form method="POST">
          <button type="submit" name="Supprimer" class="btn btn-lg btn-secondary btn-sm" value="<?php echo $donnees['NumAuteur'] ?>">Supprimer</button>
        </form>
      </div>
    </li>
  <?php } ?>
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if(isset($_POST['Supprimer'])){
      $query=$bdd->prepare('DELETE FROM auteur WHERE NumAuteur = :IdSupprimer');
