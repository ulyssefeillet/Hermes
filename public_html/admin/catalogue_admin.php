...
<button id="btn_add">
  <a href="add_book.php"> <i class="fa fa-plus">&nbsp Ajouter un nouveau livre </i></a>
</button>

<ul class="list-group mb-2 mb-sm-0 pt-1">

  <?php $reponse = $bdd->query("SELECT * FROM livre");
  while ($donnees = $reponse->fetch()) {  ?>
    <li class="list-group-item" id="<?php echo $donnees['IdLivre'] ?>">

      <form method="POST">
        <?php echo $donnees['NomLivre'] ?>
        <span id="eye_icon">
          <button type="submit" name="Visibilite" value="<?php echo $donnees['IdLivre'] ?>">
            <i
      ...
      <div class="buttons-panel">
        <form method="POST" action="modify_book.php">
          <button type="submit" name="Modifier" class="btn btn-lg btn-primary btn-sm" value="<?php echo $donnees['IdLivre'] ?>">Modifier</button>
        </form>
        <form method="POST">
          <button type="submit" name="Supprimer" class="btn btn-lg btn-secondary btn-sm" value="<?php echo $donnees['IdLivre'] ?>">Supprimer</button>
        </form>
      </div>
    </li>
  <?php } ?>
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if(isset($_POST['Supprimer'])){
      $query=$bdd->prepare('DELETE FROM livre WHERE IdLivre = :IdSupprimer');
      $query->bindValue(':IdSupprimer', $_POST['Supprimer'], PDO::PARAM_INT);
      $query->execute();

    }
    else if(isset($_POST['Visibilite'])){
      $livreMisAJour = $bdd->query("SELECT * FROM livre WHERE (IdLivre LIKE {$_POST['Visibilite']})") -> fetch();
      $query=$bdd->prepare('UPDATE livre
        SET Statut = :NewStatut
        WHERE IdLivre = :IdStatut');
        $query->bindValue(':NewStatut', abs($livreMisAJour['Statut'] - 1), PDO::PARAM_INT);
        $query->bindValue(':IdStatut', $_POST['Visibilite'], PDO::PARAM_INT);
        $query->execute();
