...
<button id="btn_add">
  <a href="add_categorie.php"> <i class="fa fa-plus">&nbsp Ajouter une nouvelle catégorie </i></a>
</button>
...
<ul class="list-group mb-2 mb-sm-0 pt-1">
  ...
  while ($donnees = $reponse->fetch()) {  ?>
    <li class="list-group-item" id="<?php echo $donnees['NumCategorie'] ?>">

      <?php echo $donnees['NomCategorie']?>
      <div class="buttons-panel">
        <form method="POST" action="modify_categorie.php">
          <button type="submit" name="Modifier" class="btn btn-lg btn-primary btn-sm" value="<?php echo $donnees['NumCategorie'] ?>">Modifier</button>
        </form>
        <form method="POST">
          <button type="submit" name="Supprimer" class="btn btn-lg btn-secondary btn-sm" value="<?php echo $donnees['NumCategorie'] ?>">Supprimer</button>
        </form>
      </div>
    </li>
  <?php } ?>
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if(isset($_POST['Supprimer'])){
      $query=$bdd->prepare('DELETE FROM categorielivre WHERE NumCategorie = :IdSupprimer');
