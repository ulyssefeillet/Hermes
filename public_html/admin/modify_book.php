...
<?php  if (isset($_POST['Modifier'])) {
  $livreMisAJour = $bdd->query("SELECT * FROM livre WHERE (IdLivre LIKE {$_POST['Modifier']})") -> fetch();
}
else {
  $livreMisAJour = $bdd->query("SELECT * FROM livre WHERE (IdLivre LIKE {$_POST['ValiderModifier']})") -> fetch();
}
?>
...
<main class="pt-5 max-lg-5">
  ...
  <?php if (!isset($_POST['ValiderModifier']))
  { ?>
    <form action = "" method = "POST" enctype = "multipart/form-data" class="border border-light p-5">
      <p class="h4 mb-4 text-left">Modifier le livre</p>
      <input class="form-control mb-4" name="Titre" id="defaultContactFormName" type="text" value="<?php echo $livreMisAJour['NomLivre'] ?>">
      ...
      <select class="browser-default custom-select mb-4" name="Auteur" id="defaultSelect">
        ...
        <?php while ($donnees = $reponse->fetch()) { ?>
          <option <?php if ($livreMisAJour['NumAuteur'] == $donnees['NumAuteur'] ) echo 'selected' ; ?> value="<?php echo $donnees['NumAuteur'] ?>"><?php echo $donnees['NomAuteur'] ?></option>
        <?php } ?>
      </select>
      ...
      <select class="browser-default custom-select mb-4" name=Format id="defaultSelect">
        <option value=""></option>
        <?php while ($donnees = $reponse->fetch()) { ?>
          <option <?php if ($livreMisAJour['NumFormat'] == $donnees['NumFormat'] ) echo 'selected' ; ?> value="<?php echo $donnees['NumFormat'] ?>"><?php echo $donnees['NomFormat'] ?></option>
        <?php } ?>
      </select>
      ...
      <select class="browser-default custom-select mb-4" name="Categorie" id="defaultSelect">
        <option value=""></option>
        <?php while ($donnees = $reponse->fetch()) { ?>
          <option <?php if ($livreMisAJour['NumCategorie'] == $donnees['NumCategorie'] ) echo 'selected' ; ?> value="<?php echo $donnees['NumCategorie'] ?>"><?php echo $donnees['NomCategorie'] ?></option>
        <?php } ?>
      </select>
      ...
      <select class="browser-default custom-select mb-4" name="Maison" id="defaultSelect">
        <option value=""></option>
        <?php while ($donnees = $reponse->fetch()) { ?>
          <option <?php if ($livreMisAJour['NumMaison'] == $donnees['NumMaison'] ) echo 'selected' ; ?> value="<?php echo $donnees['NumMaison'] ?>"><?php echo $donnees['NomMaison'] ?></option>
        <?php } ?>
      </select>

      <input class="form-control mb-4" name="Quantite" id="defaultContactFormName" type="number" step="any" placeholder="Quantité" value="<?php echo $livreMisAJour['Quantite'] ?>">
      <input class="form-control mb-4" name="Prix" id="defaultContactFormName" type="number" step="any" placeholder="Prix" value="<?php echo $livreMisAJour['Prix'] ?>">
      <label for="defaultSelect">Date de création</label>
      <input class="form-control mb-4" name="Date" id="defaultContactFormName" type="date" value="<?php echo $livreMisAJour['DateProd'] ?>">

      <label for="defaultSelect">Etat</label>
      <select class="browser-default custom-select mb-4" name="Etat" id="defaultSelect">
        <option <?php if ($livreMisAJour['Statut'] == 0) echo 'selected'; ?> value="0">Masqué</option>
        <option <?php if ($livreMisAJour['Statut'] == 1) echo 'selected'; ?> value="1">Visible</option>
      </select>

      <textarea class="form-control rounded-0" name="Description" id="exampleFormControlTextarea2" placeholder="Descriptif" rows="3"><?php echo $livreMisAJour['DescriptionLivre'] ?></textarea>
    </br>
  </br>
...
<button class="btn btn-info btn-block" name="ValiderModifier" type="submit" value="<?php echo $livreMisAJour['IdLivre'] ?>">Valider</button>
<?php
}
else {
  ...
  if(isset($_POST['ValiderModifier'])){
    ...
    {
      $query=$bdd->prepare("UPDATE livre
        SET NomLivre = :Titre,
        DescriptionLivre = :Description,
        NumFormat = :Format,
        ...
        Prix = :Prix,
        Statut = :Etat
        WHERE IdLivre LIKE {$livreMisAJour['IdLivre']}");
        ...
        if(isset($_FILES['image'])){
          ...
          move_uploaded_file($file_tmp,"Images_livre/image".$livreMisAJour['IdLivre'].$file_ext);
        ...
        if(isset($_FILES['fichier'])){
          ...
          move_uploaded_file($file_tmp,"Fichiers_livre/fichier".$livreMisAJour['IdLivre'].$file_ext);

        }

        ?> <script type="text/javascript">
        location.replace("catalogue_admin.php");
        </script> <?php
      }
      else {
        echo "\t", 'Il faut remplir tous les champs pour modifier un livre. ';
        echo "\t", '<p>Cliquez <a href="./modify_book.php">ici</a> pour recommencer</p>';
      }
